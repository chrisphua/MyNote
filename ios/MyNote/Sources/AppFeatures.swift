...
enum AppFeatures {
    ...
    /// ever shown, and StoreKit is not contacted at all.
    ///
    /// The purchasing code is kept rather than deleted — it works, it is
    /// covered by the `License` tests in `MyNoteCore`, and the cross-platform
    /// hand-off is the fiddly part to rebuild. Flipping this to `true` restores
    /// the paywall; see `docs/MONETIZATION.md` for what else that needs.
    ///
    ...
    /// after.
    static let paidFeaturesEnabled = false
}
